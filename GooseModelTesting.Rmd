...
grid_size = 1000
ext <- readShapePoly("O:/ST_GooseProject/ALMaSS/GooseModelTesting/vejlerne_outline.shp",
...
polygons <- read_tsv("C:/MSV/WorkDirectory/GooseXYDump.txt", col_types = "iiiiii") %>% 
...
fieldobs_weekly <- fieldobs %>% 
  group_by(Season, week, Species, Type) %>% 
  summarise(mean_dist = mean(dist))

# Plot the weekly average distance  
field_plot <- fieldobs_weekly %>% 
  ggplot(aes(week, mean_dist/1000)) + 
  geom_line(aes(color = Species)) + 
  ggtitle("Field observations (2015)", subtitle = "Mean weekly distance (km) from forage location to roost") +
  scale_color_colorblind() + 
  scale_x_continuous(breaks = seq(35, 75, by = 5), limits = c(35, 70)) + 
...
forage <- read_tsv("~/ALMaSS/WorkDirectory/GooseFieldForageData.txt",
                   col_types = col_types) %>% 
  dplyr::select(Season, Day, Polyref, Barnacle, Pinkfoot, Greylag) %>%
  ...
  filter(Numbers > 0) %>% 
  mutate(week = lubridate::week(as.Date(Day, origin = "2015-01-01"))) %>% 
...
sim_plot <- sim_weekly %>% 
  ggplot(aes(week, mean_dist/1000)) + 
  geom_line(aes(color = Species)) + 
  ggtitle("Simulated (average of 5 seasons)", subtitle = "Mean weekly distance (km) from forage location to roost") +
  scale_color_colorblind() + 
  scale_x_continuous(breaks = seq(35, 75, by = 5), limits = c(35, 70)) + 
  scale_y_continuous(limits = c(0, 11), breaks = seq(0,11,2)) +
  theme_fivethirtyeight()
grid.arrange(field_plot, sim_plot)
  
  
distances <- forage %>% 
  dplyr::select(Species, dist, Type) %>% 
  bind_rows(fieldobs)

RoostDistFitGL <- CalcOverlap(distances, species = 'Greylag', metric = 'dist')
RoostDistFitPF <- CalcOverlap(distances, species = 'Pinkfoot', metric = 'dist')
RoostDistFitBN <- CalcOverlap(distances, species = 'Barnacle', metric = 'dist')
...
ggplot(distances, aes(x = log10(dist), fill = Type)) +
  geom_density(alpha = 0.5, colour = NA) +
...
FieldData <- read_csv(file.path(o_drive, 'Field data/Fugledata/HabitatUseAll2014.csv'),
                      col_types = "icic") %>% 
  mutate(HabitatUse = recode(HabitatUse,
                             "Stubble undersown" = "Stubble",
                             "UnharvestedBarley" = "Stubble"),
         Month = ifelse(Month < 7, Month + 12, Month)) %>% 
  filter(HabitatUse != "Plowed") %>% 
  group_by(Month, Species, HabitatUse) %>% 
  mutate(n = sum(N)) %>% 
  dplyr::select(-N) %>% 
  dplyr::distinct()
...
cols <- c("Grass" = "#66C2A5", "Maize" = "#FC8D62", "Rape" = "#8DA0CB", "Stubble" = "#E78AC3", "WinterCereal" = "#A6D854")

fd_plot <- FieldData %>% 
  ggplot(aes(Month, fill = HabitatUse)) +
  geom_bar(aes(weight = n), position = 'fill') + 
  facet_wrap(~Species, scales = "free_x") + 
  scale_fill_manual(values = cols, guide_legend(title = "Habitat")) +
  scale_y_continuous(labels = scales::percent) + 
  ylab("Use") + 
  scale_x_continuous(breaks = 8:15, labels = c("8", "9", "10", "11", "12", "1", "2", "3")) +
  theme_fivethirtyeight() +
...
s_plot <- bind_rows(habpf, habgl, habbn) %>% 
