...
# library(slackr)
# slackrSetup(channel="#goosemodel", api_token = 'xoxp-2759174392-2762329132-3001986356-3329d2', username = 'laRs')
```
...
forage = fread(file.path(resultpath, 'GooseFieldForageData.txt'), showProgress = FALSE)

if(nrow(forage[VegTypeChr == 'WinterRye' & PreviousCrop == 'WinterRye',]) > 0){
  cat('Warning - WR following WR')
}
invisible({
forage[, Day:=Day-365]
forage[, Date:=as.Date(Day, origin = as.Date('2009-01-01'))]
forage = forage[month(Date) %in% c(8:12,1:2)]
forage = ClassifyHabitatUse(forage, species = 'goose')
forage_tb = tbl_df(forage)
# First melt the numbers:
keycols = c('Day', 'Polyref')
numbers = melt(forage[Geese > 0,], id.vars = c(keycols, 'Grain', 'Maize'),  measure.vars = c("Barnacle", "Pinkfoot", "Greylag"), variable.name = "Species", value.name = "Number")
# Then melt the grass forage
grassforage = melt(forage[Geese > 0,], id.vars = c(keycols),  measure.vars = patterns('^Grass'),
 variable.name = "Species", value.name = "KjAvailable")
grassforage[, Species:=str_sub(Species, start = 6)]
# Then merge
keycols = c(keycols, 'Species')
setkeyv(numbers, keycols)
setkeyv(grassforage, keycols)
final = merge(numbers, grassforage)
setnames(final, old = 'KjAvailable', new = 'Grass')
final = melt(final, id.vars = c(keycols, 'Number'),  measure.vars = c('Grain', 'Maize', 'Grass'),
 variable.name = "Type", value.name = "KjAvailable")
final = final[Number > 0,]
final = final[KjAvailable > 0.01,]
})

ggplot(final, aes(Day, KjAvailable)) + geom_point(alpha = 1/10, aes(size = log10(Number))) +
...
energy[,Day:=Day-365]
energy[, Date:=as.Date(Day, origin = as.Date('2009-01-01'))]
# mass = copy(energy)
# mass = mass[month(Date) %in% c(9:12,1:2)]
# mass = mass[GooseType %in% c('PFF', 'PFNB'),]
...
# fits = CalcWeightFit(mass, field)
})
 # ggplot(energy[sample(nrow(energy), 0.01*nrow(energy)),][grep('NB',GooseType),], aes(Date, Weight)) + geom_point(alpha = 1/50) + geom_smooth() + facet_wrap(~GooseType, scales = 'free_y')

# png(file = 'o:/ST_Lada/Projekter/GBB/Figures/Weights.png', height = 480*2, width = 480*2, res = 200)
# p = PlotEnergetics(SimData = mass[SeasonNumber == 1,], FieldData = field, Sample = 0.25)
# p + coord_cartesian(ylim = c(2500, 3050))
# dev.off()
invisible({
weights = fread(file.path(resultpath, 'GooseWeightStats.txt'), showProgress = FALSE)
weights = weights[MeanWeight != -1,]  
weights[,Day:=Day-365]
...
invisible({ energy[, MeanDist:=mean(FlightDistance), by = c('Day', 'GooseType') ]
energy[, MeanDist:=MeanDist/1000]
dist = unique(energy[, .(Date, GooseType, MeanDist, Season)]) })
ggplot(dist, aes(Date, MeanDist)) + geom_line(aes(color = GooseType)) + ylab('Mean distance flown per day (km)') + scale_color_brewer(palette = 'Paired') + scale_x_date(date_breaks = "1 month", date_labels = "%b") + facet_wrap(~Season, scales = 'free_x')
```

## Full time
```{r fulltime, echo=FALSE, fig.height=5, fig.width=10}
ggplot(energy, aes(Day, FullTime)) +
...
#invisible({ temp = forage[Geese > 0, .(Day, Greylag, Barnacle, Pinkfoot, Season)]
#temp = melt(temp, id.vars = c('Day', 'Season'), value.name = 'Numbers', variable.name = 'Species')
#temp = temp[Numbers != 0,]
#temp[, N:=.N, by = c('Day', 'Species')] })
# ggplot(temp, aes(Day, N)) + geom_line() + facet_grid(Season~Species, scales = 'free') + ylab('Flock numbers')
#ggplot(temp, aes(Day, N)) + geom_line() + facet_wrap(~Species, scales = 'free') + ylab('Flock numbers')
# May 1. 2017. Code converted to dplyr version and checked with setequal. 
forage_tb %>%
  filter(Geese > 0) %>%    
  select(Day, Greylag, Barnacle, Pinkfoot, Season) %>%
  reshape2::melt(id.vars = c('Day', 'Season'), value.name = 'Numbers', variable.name = 'Species') %>%
  filter(Numbers > 0) %>%
...
invisible({ leavereason = fread(file.path(resultpath, 'GooseLeaveReasonStats.txt'))
leavereason = leavereason[N > 0,]
leavereason[, Day:=Day-365]
})
if(nrow(leavereason) > 0){
ggplot(leavereason, aes(Day, N)) + geom_line(aes(color = LeaveReason)) + facet_grid(LeaveReason~SpeciesType, scales = 'free_y')
}
...
flocks = fieldobs[Month %in% c(12,1) & Hour >= 11 & Hour <= 13,]
# flocks[, .N, by = Species]  # Okay reasonable numbers
...
simflocks[,ObsDato:=as.Date(Day, origin = '2014-01-01')]
simflocks[, Day:=NULL]
melted = data.table::melt(simflocks, id.vars = 'ObsDato', variable.name = 'Species', value.name = 'Numbers')
melted = melted[Numbers != 0,]
melted[ ,ObsDato:=NULL]
melted[, Type:='Simulated']
dists = rbind(melted, flocks)
dists[, Numbers:=log10(Numbers)]
dists[, Count:='DailyMax']
...
simflocks[, ObsDato:=as.Date(Day, origin = '2014-01-01')]
simflocks = simflocks[month(ObsDato) %in% c(12,1),]
simflocks[, Day:=NULL]
melted = data.table::melt(simflocks, id.vars = 'ObsDato', variable.name = 'Species', value.name = 'Numbers')
melted = melted[Numbers != 0,]
melted[, ObsDato:=NULL]
melted[, Type:='Simulated']
dists1 = rbind(melted, flocks)
dists1[, Numbers:=log10(Numbers)]
dists1[, Count:='Timed']
# Quick fix for multiple names for one type
index = match(c("BarnacleTimed", "PinkfootTimed", "GreylagTimed"), levels(dists1$Species))
levels(dists1$Species)[index] = c('Barnacle', 'Pinkfoot', 'Greylag')
...
TheDists = rbind(dists, dists1) })
ggplot(TheDists, aes(x = Numbers, fill = Type)) +
  geom_density(alpha = 0.5, colour = NA) +
  facet_grid(Count ~ Species, scales = 'free_x') +
  theme_bw()

# png(file = 'o:/ST_Lada/Projekter/GBB/Figures/FlockSizes.png', height = 480*2.5, width = 480*4, res = 400)
# ggplot(TheDists[Count == 'Timed',], aes(x = Numbers, fill = Type)) +
#   geom_density(alpha = 0.5, colour = NA) +
#   facet_wrap(~Species, scales = 'free_x') + 
#   xlab('Flock size (log10)') + 
#   theme_bw()
# dev.off()

# Dk version:
# TheDists[, Species:=as.factor(Species)]
# levels(TheDists$Species) = c('Bramgås', 'Grågås', 'Kortnæbbet')
# TheDists[, Type:=as.factor(Type)]
# levels(TheDists$Type) = c('Feltobs', 'Simuleret')
# ggplot(TheDists[Count == "Timed",],aes(x = Numbers, fill = Type)) +
#   geom_density(alpha = 0.5, colour = NA) + 
#   facet_wrap(~Species) +
#   theme_bw() +
...
invisible({
poly = fread(paste0(resultpath, 'Vejlerne2015_polyrefs_RenumFBHBOp.txt'), skip = 1, verbose = FALSE)
fieldobs = fread('o:/ST_GooseProject/Field data/FieldobsDistancesFromRoost2016-04-25.txt')
roost = fread(paste0(resultpath,'GooseRoosts.txt'), skip = 1)

DistToNearestRoostSim = CalcDistToRoosts(roost = roost, fields = forage, polyref = poly, species = c('Pinkfoot', 'Barnacle', 'Greylag'), fieldobs = FALSE)
DistToNearestRoostSim[, Type:='Simulated']
DistToNearestRoostField = CalcDistToRoosts(roost = roost, fields = fieldobs, polyref = poly, species = c('Pinkfoot', 'Barnacle', 'Greylag'), fieldobs = TRUE)
...
ggplot(Distances, aes(x = log10(Shortest), fill = Type)) + geom_density(alpha = 0.5, colour = NA) + facet_wrap(~Species, scales = 'free_x') + theme_bw()
# dev.off()
``` 
