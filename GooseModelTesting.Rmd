...
packs = c('xtable', 'RColorBrewer', 'ralmass', 'gridExtra', 'knitr', 'lubridate', 'viridis', 'tidyverse', 'data.table', 'stringr', 'readxl', 'ggvis', "sf", "raster")
...
+ Testing some new values for following likelyhood.
...
## AOR grid
```{r, AOR_grid, echo=FALSE, fig.height=6, fig.width=10}
# Make spatial object of xy dump file
points <- read_tsv("C:/MSV/WorkDirectory/GooseXYDump.txt", col_types = "ii") %>% 
  UtmToALMaSS(points, long = "X", lat = "Y", toalmass = FALSE, map = "VejlerneBigMap") %>% 
  dplyr::select(ALong, ALat)
  
coordinates(points) <- ~ALong+ALat  
# Define template raster
r1 <- raster(nrows=24000/1000, 
             ncols=36000/1000,
             xmn=484378,
             xmx=484378+36000,
             ymn = 6335161-24000,
