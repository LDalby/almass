...
obs <- read_excel(file.path(o_drive, 'Field data/Fugledata_Obslist_20160414_clean.xlsx'), sheet = 'Observation') %>% 
  ...
  dplyr::select(PolygonID, ObsDato, ArtID, Antal) %>% 
  mutate(Species = map_chr(.x = ArtID, .f = num_id_to_chr),
         Date = ymd_hms(ObsDato),
         Week = week(Date))
...
inner_join(polys, obs, by = c("PolyID" = "PolygonID")) %>% 
  data.table::fwrite(file = pth, quote = FALSE)
...
fieldobs <- read_delim(file.path(o_drive, 'Field data/FieldobsDistancesFromRoost2016-04-25.txt'), delim = ' ', col_types = col_types_field)
