...
obs <- read_excel(file.path(o_drive, 'Field data/Fugledata_Obslist_20160414_clean.xlsx'), sheet = 'Observation') %>% 
  ...
  dplyr::select(PolygonID, Year, Month, ArtID, Antal) %>% 
  mutate(Species = map_chr(.x = ArtID, .f = SwapType))
...
pth <- paste0('o:/ST_GooseProject/Field data/FieldobsDistancesFromRoost', Sys.Date(), '.txt')
fwrite(convertedobs, file = pth, row.names = FALSE, quote = FALSE)
