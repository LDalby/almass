...
allres = fread("/home/lars/temp/GooseParameterFitting_2017-02-02.txt") 	
invisible({parameters = allres[,unique(Parameter)]
 	defaultvalues = rep(NA, length(parameters))
 	for(i in seq_along(parameters)){
 	  defaultvalues[i] = GetParamValue(cfg, parameters[i])
 	}
 	dt = data.table('Parameter' = parameters, 'Defaultvalues' = defaultvalues, key = 'Parameter')
  ...
  DT = DT[Parameter != 'GOOSE_MEM_MINMEMVALUE',]
    # DT[, Standardized:=Standardized-1]
  parameters = DT[,unique(Parameter)]})
for(i in seq_along(parameters)) {
  paramvalue = GetParamValue(cfg, parameters[i])
...
paramvalue = GetParamValue(cfg, "GOOSE_MEM_MINMEMVALUE")
...
for (i in seq_along(allres[,unique(Parameter)])) {
	res = allres[Parameter == unique(Parameter)[i],]
 	...
 	paramvalue = GetParamValue(cfg, param)
